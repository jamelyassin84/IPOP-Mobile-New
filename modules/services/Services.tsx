				...
				{services.map((service: any, index: number) => (
					<View key={index} style={style.container}>
						<View style={{ paddingRight: 10, flex: 1 }}>
							<Text style={[style.title, { color: Colors[colorScheme].text, fontWeight: '700' }]}>{service.title}</Text>
							{service.offers.map((offer: any, index: number) => (
								<Text key={index} style={{ color: '#0D47A1', fontSize: 12, marginLeft: 15, marginTop: 10 }}>
									{offer.title}
								</Text>
							))}
						</View>
